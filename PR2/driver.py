...
def tape_calc(global_network,batch_obs, batch_goals, batch_rewards, batch_actions,  batch_valids, batch_messages, batch_masks, batch_tentatives,batch_weights, world_optimizer, policy_optimizer):
    ...
    global_network.pi_out.trainable_variables
)

    variables_except_for_actor = (
    # --- Encoder ---
    global_network.conv_entry.trainable_variables +
    ...
    global_network.mha_ln1.trainable_variables +
    global_network.mha_ff.trainable_variables +
    global_network.mha_ln2.trainable_variables +
    global_network.comm_out.trainable_variables +

    # --- Dynamics ---
    global_network.dyn_embed.trainable_variables +
    ...
    global_network.rew_res1.trainable_variables +
    global_network.rew_out.trainable_variables +

    # --- Value (Q1) ---
    global_network.q1_embed.trainable_variables +
    global_network.q1_res1.trainable_variables +
    global_network.q1_out.trainable_variables +

    # --- Value (Q2) ---
    global_network.q2_embed.trainable_variables +
    global_network.q2_res1.trainable_variables +
    global_network.q2_out.trainable_variables
)
    ...
    batch_latent_targets=global_network.communication(batch_curr_own_message,batch_replaced_messages,batch_masks[:,1:])
    ...
    with tf.GradientTape() as tape:
        ...
        latent_init=global_network.communication(curr_own_message,messages,batch_masks[:,0:1])
    ...
    with tf.GradientTape() as tape:
        ...
        # 2. 確率分布 (π) を計算
        probs = tf.nn.softmax(logits, axis=-1)  # [B, H, A]

        # 3. 全アクションパターンの Q 値を 3次元を維持して計算
        B = tf.shape(latents_for_policy)[0]
        H = tf.shape(latents_for_policy)[1]
        A = tf.shape(probs)[-1] # アクション数

        # Latentを全アクション分コピー [B, H, A, Dim]
        latent_tiled = tf.expand_dims(latents_for_policy, axis=2)
        latent_tiled = tf.tile(latent_tiled, [1, 1, A, 1])
        
        # 全アクションの one-hot 行列を作成 [B, H, A, A]
        all_actions = tf.eye(A)
        all_actions = tf.reshape(all_actions, [1, 1, A, A])
        all_actions = tf.tile(all_actions, [B, H, 1, 1])

        # 【重要】Qネットワークが期待する 3次元 (Batch, Seq, Dim) に整形
        # ここでは (B*H) をバッチとして扱い、A (全アクション) をシーケンスとして見せる、
        # あるいは (B) をバッチ、(H*A) をシーケンスとして扱います。
        # 今回は (B*H) をひとまとめにするのが安全です。
        latent_input = tf.reshape(latent_tiled, [B * H, A, RNN_SIZE])
        action_input = tf.reshape(all_actions, [B * H, A, A])

        # Q 値の計算 [B*H, A, 1]
        q1_all = global_network.q1(latent_input, action_input)
        q2_all = global_network.q2(latent_input, action_input)
        
        q_min_all = tf.math.minimum(q1_all, q2_all)
        # もとの [B, H, A] に戻す
        q_min_all = tf.reshape(q_min_all, [B, H, A])

        # 4. 期待値 (Expectation) の計算
        # Σ π(a|s) * Q(s, a)
        expected_q = tf.reduce_sum(probs * q_min_all, axis=-1) # [B, H]
        ...
        while True:
            ...
            if obsResults and goalsResults and actionsResults and rewardsResults and validsResults and messagesResults and masksResults and tentativesResults:
                ...
                if curr_episode>(random_term-1): #random_term個分が終わったタイミングから学習を始めたい。
                    for i in range(num_step//4):#(2+ int((NUM_THREADS-2)*max([min([(-5.0+global_mean_finishes)/40.0, 1.0]), 0.0])))//4): #max_episode_length*NUM_THREADS//4
