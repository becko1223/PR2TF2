...
class Runner(object):
    ...
    def multiThreadedJob(self, episodeNumber, mean_finishes):
        ...
        if not is_imitation:
            ...
            avg_perf_metrics = np.mean(perf_metrics[:, :7], axis=0)
            episode_reward = np.sum(perf_metrics[:,7])
            targets_done = np.sum(perf_metrics[:, 8])
    ...
    def job(self, global_weights, episodeNumber, mean_finishes):
        try:
            ...
            if self.metaAgentID < NUM_IL_META_AGENTS:
                ...
                obsResults, goalsResults, actionsResults, rewardsResults,  validsResults, metrics, is_imitation = self.multiThreadedJob(episodeNumber, mean_finishes)
            ...
            result= obsResults, goalsResults, actionsResults, rewardsResults, validsResults, metrics, info
            return result
