...
class Runner(object):
    ...
    def multiThreadedJob(self, episodeNumber,global_weights):
        ...
        avg_loss_metrics = list(np.nanmean(np.array(loss_metrics), axis=0))
        ...
        if not is_imitation:
            ...
            avg_perf_metrics = np.nanmean(perf_metrics[:, :4], axis=0)
            episode_reward = np.nansum(perf_metrics[:,4])
            targets_done = np.nansum(perf_metrics[:, 5])
...
@ray.remote(num_cpus=max([(cpu*3)//19,1]), num_gpus= 1.0 / (NUM_META_AGENTS - NUM_IL_META_AGENTS + 1))
class RLRunner(Runner):
    def __init__(self, metaAgentID):        
        super().__init__(metaAgentID)


@ray.remote(num_cpus=max([(cpu*1)//19,1]), num_gpus=0,memory=4_000_000_000)
